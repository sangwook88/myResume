...
- [요소/프로젝트-인덱스.md](요소/프로젝트-인덱스.md) — 한 프로젝트의 index.md 렌더(요약·역할·스택·아키텍처·성과·포인트 목록). **v4: 압축 아키텍처 도식 SVG 임베드**
- [요소/포폴-포인트-상세.md](요소/포폴-포인트-상세.md) — 포폴 포인트 1개의 9섹션(STAR+ADR) + Evidence 렌더. **v4: visible만 렌더(invidence 미수신·미노출)**
...
- **(v4) invidence 층(디테일·코드)의 저장·추출·챗봇 소비** — be/point(추출)·be/chat(소비). fe/browse는 애초에 받지 않음.
- **(v4) 도식 소스(typst) 저작·SVG 컴파일** — pofol(저작)·be/project(빌드 컴파일). fe/browse는 완성된 SVG 애셋을 임베드만.
