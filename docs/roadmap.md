...
**프로토타입 완료 (2026-07-12).**   <!-- v1·v2 마감(2026-07-08). v3는 프로토타입 범위로 넣을 인라인 선택-질문까지 구현하고 종료 — 자동화·RAG 계열(git 분석·raw 인박스·wiki 저작 엔진·검색 RAG·watcher)은 "자동 소스가 실제로 필요해질 때"까지 의도적 게이트 유지(미착수가 아니라 보류). 아래 표의 ✅는 버전 귀속 표시. -->
